...
def process_questions(doc, extracted_text, questions):
    ...
    for question in questions:
        doc.add_paragraph(f"{question['section']}", style="Heading 1")

        if question["search_pattern"] in extracted_text:
            doc.add_paragraph(question["message_found"], style="Heading 2")
...
def process_zip(zip_path, output_docx, yaml_path):
    ...
    for file_name in sorted(os.listdir(output_folder)):
        ...
        if group:
            doc.add_paragraph(group["message_if_identifier_found"], style="Heading 3")
